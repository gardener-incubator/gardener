...
// Please read!
// Prerequisites:
//  - existing etcd cluster that is accessible from the runtime cluster
//  - there must be a Kubernetes API server that is properly set up for API aggregation
//  - The kubeconfig for this API server has to be configured either in the field RuntimeCluster or
//    in the field VirtualGarden in the import configuration
// This component extends the given Kubernetes API server with the Gardener API server that serves
// all of the Gardener resource groups (e.g for Shoots)
// To setup API server aggregation, the Kubernetes API server has to be properly configured with configuration flags.
// For more information how to configure the aggregation layer see:
